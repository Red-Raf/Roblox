...
t1.SkinChangerTab:Section({
	Title = "Skin Changer",
	Icon = "shirt"
})
t1.SkinChangerTab:Divider()

t1.SkinChangerTab:Paragraph({
	Title = "Skin Changer Toggle",
	Desc = "Увімкніть щоб завантажити скрипт скінченджера. Вимкніть щоб закрити меню скінченджера (скіни залишаться).",
	Image = "info",
	ImageSize = 20
})

t1.SkinChangerTab:Toggle({
	Title = "Enable Skin Changer",
	Desc = "Завантажує / вивантажує скрипт скінченджера",
	Value = false,
	Callback = function(enabled)
		if enabled then
			if not skinChangerLoaded then
				skinChangerLoaded = true
				loadstring(game:HttpGet("https://raw.githubusercontent.com/Dastway/DustWayHub/refs/heads/main/MM2_skinchanger.lua"))()
				_Raw:Notify({
					Title = "Skin Changer",
					Content = "Скрипт скінченджера завантажено!",
					Duration = 5,
					Icon = "check",
					IconThemed = true
				})
			...
			if skinChangerLoaded then
				...
				pcall(function()
					if getgenv().SkinChangerWindow then
						getgenv().SkinChangerWindow:Destroy()
						getgenv().SkinChangerWindow = nil
					end
				end)
				pcall(function()
					if getgenv().SkinChangerGui then
						getgenv().SkinChangerGui:Destroy()
						getgenv().SkinChangerGui = nil
					end
				end)
				pcall(function()
					for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
						if gui.Name:lower():find("skin") then
							gui:Destroy()
						end
					end
				end)
				...
				_Raw:Notify({
					Title = "Skin Changer",
					Content = "Меню скінченджера закрито. Скіни залишились.",
					Duration = 5,
					Icon = "x",
					IconThemed = true
				})
...
t1.SkinChangerTab:Button({
	Title = "Reload Skin Changer",
	Desc = "Перезавантажити скрипт скінченджера",
	Icon = "refresh-cw",
	Callback = function()
		skinChangerLoaded = true
		loadstring(game:HttpGet("https://raw.githubusercontent.com/Dastway/DustWayHub/refs/heads/main/MM2_skinchanger.lua"))()
		_Raw:Notify({
			Title = "Skin Changer",
			Content = "Скрипт перезавантажено!",
			Duration = 5,
			Icon = "refresh-cw",
			IconThemed = true
		})
...
t1.SkinChangerTab:Button({
	Title = "Close Skin Changer Menu",
	Desc = "Закрити вікно скінченджера (скіни залишаться)",
	Icon = "x-circle",
	Callback = function()
		pcall(function()
			if getgenv().SkinChangerWindow then
		...
		_Raw:Notify({
			Title = "Skin Changer",
			Content = "Меню скінченджера закрито.",
			Duration = 3,
			Icon = "x",
			IconThemed = true
		})
...
local function u176(p131)
	...
	local _Database = ReplicatedStorage3.Database:FindFirstChild("Sync") and ReplicatedStorage3.Database.Sync:FindFirstChild("Item")
...
-- ============================================================
-- ============================================================
-- [DUSTWAY EXTENDED] Silent Aim + Shot Marker + Auto Gun Logic
-- + Shoot Murderer Button (Disabled by default) + WallShot
-- ============================================================
-- ============================================================
...
local EXTF = {
	...
	Hitmarker         = false,
	HitSound          = true,
	HitColor          = Color3.fromRGB(255, 80, 80),
	HitSize           = 32,
	AutoGun           = false,
	AutoGunRadius     = 30,
	AutoShootMurd     = false,
...
local extfLastVel = {}
local extfHitConns = {}
local extfSilentCd = 0
local extfAutoGunThread = nil
local extfAutoShootThread = nil
local extfWallShotThread = nil

local extfShootBtn = nil
local extfShootGui = nil

local function extfPredict(part)
	if not part or not part:IsA("BasePart") then
		return (part and part.Position) or Vector3.zero
	end
	if not EXTF.PredictionOn then return part.Position end
	...
	local state  = hum and hum:GetState() or Enum.HumanoidStateType.None

	local dist = (hrp.Position - part.Position).Magnitude
	local ping = LocalPlayer:GetNetworkPing() or 0
	local dt   = math.clamp(dist / math.max(hrp.AssemblyLinearVelocity.Magnitude + 800, 800) * 0.6 + ping, 0.005, 0.12)

	local vel = part.AssemblyLinearVelocity
	if not extfLastVel[part] then extfLastVel[part] = vel end

	local accel   = (vel - extfLastVel[part]).Magnitude
	local lerpA   = accel > 50 and 0.8 or 0.35
	local smoothV = extfLastVel[part]:Lerp(vel, lerpA)
	extfLastVel[part] = smoothV

	local predicted = part.Position + smoothV * dt
	...
	if isAir then
		predicted += Vector3.new(0, workspace.Gravity * dt * dt * 0.12, 0)
		if smoothV.Y > 0 then
			predicted += Vector3.new(0, smoothV.Y * dt * 0.08, 0)
		end
	end

	local yDiff  = predicted.Y - part.Position.Y
	local yClamp = isAir
		and math.clamp(yDiff, -dist * 0.08, dist * 0.12)
		or  math.clamp(yDiff, -dist * 0.03, dist * 0.05)
	...
	local finalPos = Vector3.new(
		predicted.X,
		part.Position.Y + yClamp + headOffset * 0.25,
		predicted.Z
	)

	local maxOffset = math.max(dist * 0.25, 8)
	local diff = finalPos - part.Position
	if diff.Magnitude > maxOffset then
		finalPos = part.Position + diff.Unit * maxOffset
	end

	return finalPos
end

local function extfGetRole(player)
	local info = t4.Roles and t4.Roles[player.Name]
	return info and info.Role or "Unknown"
end

local function extfIsDead(player)
	local info = t4.Roles and t4.Roles[player.Name]
	return info and info.Dead == true
end

local function extfGetMurderer()
	if t4.Murderer and t4.Murderer ~= "Unknown" then
		local p = Players:FindFirstChild(t4.Murderer)
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= LocalPlayer then
			local info = t4.Roles and t4.Roles[p.Name]
...
local function extfLocalRole()
	return extfGetRole(LocalPlayer)
end

local function extfSilentShoot()
	if not EXTF.SilentAim then return end

	local role = extfLocalRole()
	if role ~= "Sheriff" and role ~= "Hero" then return end
	if tick() - extfSilentCd < 0.1 then return end
	extfSilentCd = tick()

	local murderer = extfGetMurderer()
	if not murderer then return end

	local mChar = murderer.Character
	if not mChar then return end

	local target = mChar:FindFirstChild("UpperTorso")
	...
	local wsGun = charFolder:FindFirstChild("Gun")
	if not wsGun then
		local t = tick()
		while (not wsGun) and (tick() - t < 0.5) do
			task.wait()
			wsGun = charFolder:FindFirstChild("Gun")
		end
	end
	if not wsGun then return end

	local shoot = wsGun:FindFirstChild("Shoot", true)
	if not shoot then return end

	local aimPos = extfPredict(target)
...
local function extfWallShot()
	if not EXTF.WallShot then return end
	...
	if not wsGun then
		local t = tick()
		while (not wsGun) and (tick() - t < 1.5) do
			task.wait()
			wsGun = charFolder:FindFirstChild("Gun")
		end
	end
...
local function extfHookGun(gun)
	if not gun or not gun:IsA("Tool") then return end
	if gun:GetAttribute("EXTF_Hooked") then return end
	gun:SetAttribute("EXTF_Hooked", true)
	gun.Activated:Connect(function()
		if EXTF.SilentAim then
			task.spawn(extfSilentShoot)
		end
	end)
end

local function extfHookChar(char)
	if not char then return end
	for _, c in ipairs(char:GetChildren()) do
		if c.Name == "Gun" and c:IsA("Tool") then extfHookGun(c) end
	end
...
if LocalPlayer.Character then extfHookChar(LocalPlayer.Character) end
LocalPlayer.CharacterAdded:Connect(extfHookChar)
...
local extfHitGui = Instance.new("ScreenGui")
extfHitGui.Name = "EXTF_Hitmarker"
extfHitGui.ResetOnSpawn = false
extfHitGui.IgnoreGuiInset = true
extfHitGui.DisplayOrder = 999999
local ok_gui, parentGui = pcall(function()
	return (gethui and gethui()) or game:GetService("CoreGui")
end)
extfHitGui.Parent = ok_gui and parentGui or LocalPlayer:WaitForChild("PlayerGui")

local extfHitFrame = Instance.new("Frame")
extfHitFrame.Name = "Mark"
extfHitFrame.AnchorPoint = Vector2.new(0.5, 0.5)
extfHitFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
extfHitFrame.Size = UDim2.new(0, EXTF.HitSize, 0, EXTF.HitSize)
extfHitFrame.BackgroundTransparency = 1
extfHitFrame.Visible = false
extfHitFrame.Parent = extfHitGui

local extfHitLines = {}
for i = 1, 4 do
	local line = Instance.new("Frame")
	line.BackgroundColor3 = EXTF.HitColor
	line.BorderSizePixel = 0
	line.AnchorPoint = Vector2.new(0.5, 0.5)
	line.Position = UDim2.new(0.5, 0, 0.5, 0)
	line.Size = UDim2.new(0, 3, 0, 10)
	line.Parent = extfHitFrame
	extfHitLines[i] = line
end

local function extfLayoutHit()
	local s = EXTF.HitSize
	local positions = {
		{ x = -s*0.25, y = -s*0.25, rot = -45 },
		{ x =  s*0.25, y = -s*0.25, rot =  45 },
		{ x = -s*0.25, y =  s*0.25, rot =  45 },
		{ x =  s*0.25, y =  s*0.25, rot = -45 },
	}
	for i, p in ipairs(positions) do
		extfHitLines[i].Position = UDim2.new(0.5, p.x, 0.5, p.y)
		extfHitLines[i].Rotation = p.rot
		extfHitLines[i].Size = UDim2.new(0, 3, 0, math.floor(EXTF.HitSize * 0.28))
	end
end
extfLayoutHit()

local extfHitSound = Instance.new("Sound")
extfHitSound.SoundId = "rbxassetid://225320558"
extfHitSound.Volume = 0.5
extfHitSound.Parent = extfHitGui

local TweenService_EXTF = game:GetService("TweenService")

local function extfShowHit()
	if not EXTF.Hitmarker then return end

	extfHitFrame.Visible = true
	extfHitFrame.Size = UDim2.new(0, EXTF.HitSize * 1.4, 0, EXTF.HitSize * 1.4)

	for _, l in ipairs(extfHitLines) do
		l.BackgroundColor3 = EXTF.HitColor
		l.BackgroundTransparency = 0
	end

	if EXTF.HitSound and extfHitSound then
		extfHitSound:Play()
	end

	TweenService_EXTF:Create(
		extfHitFrame,
		TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
		{ Size = UDim2.new(0, EXTF.HitSize, 0, EXTF.HitSize) }
	):Play()

	task.delay(0.35, function()
		for _, l in ipairs(extfHitLines) do
			TweenService_EXTF:Create(l, TweenInfo.new(0.2), { BackgroundTransparency = 1 }):Play()
		end
...
local function extfWatchHP()
	for name, conn in pairs(extfHitConns) do
		if not Players:FindFirstChild(name) then
			pcall(function() conn:Disconnect() end)
			extfHitConns[name] = nil
		end
	end

	local m = extfGetMurderer()
	if not m or not m.Character then return end
	if extfHitConns[m.Name] then return end
...
local function extfFindGunDrop(originPos, radius)
	local nearest, nearestDist = nil, radius
	for _, obj in ipairs(workspace:GetDescendants()) do
		if obj.Name == "GunDrop" and obj:IsA("BasePart") and obj.Parent then
			local d = (obj.Position - originPos).Magnitude
...
local function extfAutoGunLoop()
	while EXTF.AutoGun do
		task.wait(0.12)
		pcall(function()
			local char = LocalPlayer.Character
			local hrp  = char and char:FindFirstChild("HumanoidRootPart")
			...
			local drop = extfFindGunDrop(hrp.Position, EXTF.AutoGunRadius)
	...
	extfAutoGunThread = nil
end

local function extfStartAutoGun()
	if extfAutoGunThread then return end
	extfAutoGunThread = task.spawn(extfAutoGunLoop)
end

local function extfAutoShootLoop()
	while EXTF.AutoShootMurd do
		task.wait(EXTF.AutoShootDelay)
		pcall(extfSilentShoot)
	end
	extfAutoShootThread = nil
end

local function extfWallShotLoop()
	while EXTF.WallShot do
		task.wait(EXTF.WallShotDelay)
		pcall(extfWallShot)
	end
	extfWallShotThread = nil
end

local function extfCreateShootButton()
	local parent = (gethui and gethui()) or game:GetService("CoreGui")

	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "DustWay_ShootMurderer"
	screenGui.ResetOnSpawn = false
	...
	shootBtn.Name = "ShootMurdererBtn"
	shootBtn.Size = UDim2.new(0, EXTF.ShootBtnW, 0, EXTF.ShootBtnH)
	shootBtn.Position = UDim2.new(EXTF.ShootBtnX, -EXTF.ShootBtnW/2, EXTF.ShootBtnY, -EXTF.ShootBtnH/2)
	shootBtn.Text = "Shoot Murderer"
	shootBtn.BackgroundTransparency = 0.15
	shootBtn.BackgroundColor3 = Color3.fromRGB(25, 40, 60)
	...
	shootBtn.Visible = EXTF.ShootBtnEnabled
	shootBtn.Active = true
	shootBtn.Parent = screenGui

	Instance.new("UICorner", shootBtn).CornerRadius = UDim.new(0, 10)

	local stroke = Instance.new("UIStroke", shootBtn)
	stroke.Thickness = 2.5
	stroke.Color = Color3.fromRGB(80, 180, 210)

	local gradient = Instance.new("UIGradient", stroke)
	gradient.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(25, 69, 87)),
		ColorSequenceKeypoint.new(0.35, Color3.fromRGB(131, 224, 255)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(131, 224, 255)),
		ColorSequenceKeypoint.new(0.65, Color3.fromRGB(25, 69, 87)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(25, 69, 87)),
	})

	task.spawn(function()
		local angle = 0
		while shootBtn.Parent do
			angle = (angle + 2) % 360
	...
	local ripple = Instance.new("Frame", shootBtn)
	ripple.Size = UDim2.new(0, 0, 0, 0)
	ripple.Position = UDim2.new(0.5, 0, 0.5, 0)
	ripple.AnchorPoint = Vector2.new(0.5, 0.5)
	ripple.BackgroundColor3 = Color3.fromRGB(235, 149, 22)
	...
	local function rippleEffect()
		...
		ripple.Visible = true
		local tw = TweenService_EXTF:Create(ripple,
	...
	shootBtn.MouseButton1Click:Connect(function()
		...
		if EXTF.SilentAim then
			task.spawn(extfSilentShoot)
		elseif EXTF.WallShot then
			task.spawn(extfWallShot)
		else
			local old = EXTF.SilentAim
			EXTF.SilentAim = true
			task.spawn(extfSilentShoot)
			task.wait(0.3)
			EXTF.SilentAim = old
	...
	extfShootBtn = shootBtn
	extfShootGui = screenGui

	return screenGui, shootBtn
end

do
	local tab = t1.ShotTab
	tab:Section({ Title = "DustWay Extended Combat", Icon = "crosshair" })
	tab:Divider()

	tab:Toggle({
		Title = "Silent Aim",
		Desc = "Автоматично підмінює CFrame при пострілі",
		Value = false,
		Callback = function(v) EXTF.SilentAim = v end
	})
	tab:Toggle({
		Title = "Prediction",
		Desc = "Передбачення руху мурдера",
		Value = true,
		Callback = function(v) EXTF.PredictionOn = v end
	})
	tab:Toggle({
		Title = "Auto Shoot Murderer (Loop)",
		Desc = "Постійно стріляє у мурдера",
		Value = false,
		Callback = function(v)
			EXTF.AutoShootMurd = v
			if v and not extfAutoShootThread then
				extfAutoShootThread = task.spawn(extfAutoShootLoop)
			end
		end
	})
	tab:Slider({
		...
		Callback = function(v) EXTF.AutoShootDelay = v end
	})

	tab:Divider()

	tab:Toggle({
		Title = "WallShot (Shoot through walls)",
		Desc = "Стріляє крізь стіни по мурдеру",
		Value = false,
		Callback = function(v)
			EXTF.WallShot = v
			if v and not extfWallShotThread then
				extfWallShotThread = task.spawn(extfWallShotLoop)
			end
		end
	})
	tab:Slider({
		Title = "WallShot Delay",
		Step = 0.05,
		Value = { Min = 0.05, Max = 1, Default = 0.1 },
		Callback = function(v) EXTF.WallShotDelay = v end
	})

	tab:Divider()

	tab:Toggle({
		Title = "Shot Marker (Hitmarker)",
		Desc = "Показує маркер влучання",
		Value = false,
		Callback = function(v) EXTF.Hitmarker = v end
	})
	tab:Toggle({
		Title = "Hitmarker Sound",
		Value = true,
		Callback = function(v) EXTF.HitSound = v end
	})
	tab:Slider({
	...
	tab:Toggle({
		Title = "Auto Grab Gun (Loop)",
		Desc = "Автоматично підбирає GunDrop",
		Value = false,
		Callback = function(v)
			EXTF.AutoGun = v
			if v then extfStartAutoGun() end
		end
	})
	tab:Slider({
		Title = "Auto Grab Radius",
		Step = 5,
		Value = { Min = 10, Max = 200, Default = 30 },
		Callback = function(v) EXTF.AutoGunRadius = v end
	})
	tab:Button({
		Title = "Grab Gun Now",
		Icon = "hand",
		Callback = function()
			pcall(function()
				...
				local hrp  = char and char:FindFirstChild("HumanoidRootPart")
				if not hrp then return end
				local drop = extfFindGunDrop(hrp.Position, 500)
				if drop and firetouchinterest then
					firetouchinterest(hrp, drop, 0)
					task.wait(0.02)
					firetouchinterest(hrp, drop, 1)
				end
	...
	tab:Section({ Title = "Shoot Murderer Button", Icon = "target" })

	tab:Toggle({
		Title = "Show Shoot Button",
		Desc = "Показує кнопку Shoot Murderer на екрані",
		Value = false,
		Callback = function(v)
			EXTF.ShootBtnEnabled = v
			if extfShootBtn then
				extfShootBtn.Visible = v
			end
		end
	})

	tab:Button({
		Title = "Показати кнопку зараз",
		Icon = "eye",
		Callback = function()
			EXTF.ShootBtnEnabled = true
			if extfShootBtn then
				extfShootBtn.Visible = true
			end
		end
	})

	tab:Slider({
		Title = "Button Position X",
		Desc = "Позиція по горизонталі (0 = ліво, 100 = право)",
		Step = 1,
		Value = { Min = 0, Max = 100, Default = 50 },
		Callback = function(v)
			EXTF.ShootBtnX = v / 100
			if extfShootBtn then
				extfShootBtn.Position = UDim2.new(EXTF.ShootBtnX, -EXTF.ShootBtnW/2, EXTF.ShootBtnY, -EXTF.ShootBtnH/2)
			end
		end
	})

	tab:Slider({
		Title = "Button Position Y",
		Desc = "Позиція по вертикалі (0 = верх, 100 = низ)",
		Step = 1,
		Value = { Min = 0, Max = 100, Default = 60 },
		Callback = function(v)
			EXTF.ShootBtnY = v / 100
			if extfShootBtn then
				extfShootBtn.Position = UDim2.new(EXTF.ShootBtnX, -EXTF.ShootBtnW/2, EXTF.ShootBtnY, -EXTF.ShootBtnH/2)
			end
		end
	...
	tab:Slider({
		Title = "Button Width",
		Step = 5,
		Value = { Min = 80, Max = 400, Default = 200 },
		Callback = function(v)
			EXTF.ShootBtnW = v
			if extfShootBtn then
				extfShootBtn.Size = UDim2.new(0, v, 0, EXTF.ShootBtnH)
				extfShootBtn.Position = UDim2.new(EXTF.ShootBtnX, -v/2, EXTF.ShootBtnY, -EXTF.ShootBtnH/2)
			end
		end
	})
	...
	tab:Slider({
		...
		Callback = function(v)
			EXTF.ShootBtnH = v
			if extfShootBtn then
				extfShootBtn.Size = UDim2.new(0, EXTF.ShootBtnW, 0, v)
				extfShootBtn.Position = UDim2.new(EXTF.ShootBtnX, -EXTF.ShootBtnW/2, EXTF.ShootBtnY, -v/2)
			end
		end
	})
end

-- Створюємо кнопку ПІСЛЯ UI
extfCreateShootButton()
...
_Raw:Notify({
	Title = "DustWay Extended",
	Content = "Silent Aim + WallShot + Shot Marker + Auto Gun завантажено (Shot Tab)",
	Duration = 6,
	Icon = "crosshair",
	IconThemed = true
})

print("[DustWay] EXTF Extended Combat loaded (Shoot Button ready)")